    ...
    <div style={{ position: "relative", display: "inline-block" }}>
      <select
        ...
        style={{
          ...
          fontFamily: "'Jost',sans-serif", borderRadius: 20,
          border: "1px solid " + (current ? current.color + "55" : "#e5e7eb"),
          ...
          transition: "all .2s", opacity: props.loading ? 0.6 : 1,
        }}
      >
        <option value="">-- unset --</option>
        {PAY_OPTIONS.map(function(o) {
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 13, color: MUTED, lineHeight: 1.6, fontWeight: 300 }}>
            You are about to permanently delete the RSVP entry for
          </p>
          <div style={{ fontWeight: 600, color: NAVY, fontSize: 15, marginTop: 6 }}>
            {props.guest.full_name}
          </div>
          {props.guest.email && (
            <div style={{ fontSize: 12, color: MUTED, marginTop: 2 }}>{props.guest.email}</div>
          )}
          <p style={{ fontSize: 12, color: "#dc2626", marginTop: 12, fontWeight: 300 }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <Btn variant="ghost" onClick={props.onCancel} style={{ flex: 1 }} disabled={props.deleting}>
            Cancel
          </Btn>
  ...
  return (
    <button
      ...
      style={{
        ...
        border: "0.5px solid " + (hov ? "#fca5a5" : NAVY + "18"),
        borderRadius: 6,
        cursor: "pointer",
        padding: "5px 7px",
        transition: "all .2s",
...
function Dashboard(props) {
  ...
  function handleDeleteClick(guest) {
  ...
  function handleDeleteConfirm() {
    ...
    dbDelete(deleteTarget.id, token)
      .then(function() {
  ...
  const asoebiAll   = guests.filter(function(g) { return g.asoebi_choice === "yes" || g.asoebi_choice === "yes_paid"; });
  const paid        = guests.filter(function(g) { return g.payment_status === "paid"; });
  const pending     = guests.filter(function(g) { return g.payment_status === "pending"; });

  const filtered = guests.filter(function(g) {
    const q = search.toLowerCase();
    const matchSearch = !q || (g.full_name && g.full_name.toLowerCase().includes(q)) || (g.email && g.email.toLowerCase().includes(q));
  ...
  function exportCSV() {
    ...
    const rows = [
      ...
      ...guests.map(function(g) {
        return cols.map(function(c) { return '"' + (g[c] != null ? String(g[c]) : "") + '"'; }).join(",");
      })
    ];
  ...
  function asoLabel(v) { return v === "yes_paid" ? "Yes - paid" : v === "yes" ? "Yes - pending" : v === "no" ? "No" : "--"; }
  ...
  const tdStyle = { padding: "12px 16px", borderBottom: "0.5px solid " + GOLD + "15" };
      ...
      <div style={{ background: NAVY, padding: "0 32px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Btn variant="ghost" onClick={props.onLogout} style={{ color: GOLD + "cc", borderColor: GOLD + "33", fontSize: 11, padding: "8px 16px" }}>
              Logout
            </Btn>
          </div>
        </div>
          ...
          <>
            <div style={{ background: "#fff", border: "0.5px solid " + GOLD + "33", borderRadius: 12, padding: "16px 20px", marginBottom: 18, display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {[["all","All"],["attending","Attending"],["not_attending","Not Attending"],["asoebi","Asoebi"],["paid","Paid"],["pending","Pending"],["contributed","Contributed"]].map(function(item) {
                  const v = item[0], l = item[1];
                  return (
                    <button key={v} onClick={function() { setFilter(v); }} style={{
                      ...
                      transition: "all .2s", fontFamily: "'Jost',sans-serif", letterSpacing: "0.04em",
                    }}>{l}</button>
                  );
                })}
              </div>
              <input
                ...
                style={{ marginLeft: "auto", padding: "8px 14px", fontSize: 13, border: "0.5px solid " + NAVY + "22", borderRadius: 8, fontFamily: "'Jost',sans-serif", color: NAVY, minWidth: 220, outline: "none", background: "#fff" }}
              />
            </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead>
                      <tr style={{ background: CREAM2 }}>
                        {["Name","Email","Attending","Contribution","Asoebi","Item","Payment","Date",""].map(function(h) {
                      ...
                      {filtered.map(function(g, i) {
                        return (
                          <tr key={g.id} style={{ background: i % 2 === 0 ? "#fff" : CREAM + "66" }}>
                            <td style={Object.assign({}, tdStyle, { fontWeight: 500, color: NAVY })}>{g.full_name}</td>
                            <td style={Object.assign({}, tdStyle, { color: MUTED })}>{g.email || "--"}</td>
                            <td style={tdStyle}>
                              <Badge label={attendingLabel(g.attending)} color={attendingColor(g.attending)} />
                            </td>
                            <td style={tdStyle}>
                              {g.contribution_choice
                                ? <Badge label={g.contribution_choice} color={g.contribution_choice === "yes" ? "gold" : "gray"} />
                                : "--"}
                            </td>
                            <td style={tdStyle}>
                              {g.asoebi_choice
                                ? <Badge label={asoLabel(g.asoebi_choice)} color={asoColor(g.asoebi_choice)} />
                                : "--"}
                            </td>
                            <td style={Object.assign({}, tdStyle, { color: MUTED })}>{itemLabel[g.asoebi_item] || "--"}</td>
                            <td style={tdStyle}>
                              <PayStatusDropdown
                                value={g.payment_status}
                                loading={updating === g.id}
                                onChange={function(status) { updatePaymentStatus(g.id, status); }}
                              />
                            </td>
                            <td style={Object.assign({}, tdStyle, { color: MUTED, fontSize: 12, whiteSpace: "nowrap" })}>
                            ...
                            <td style={Object.assign({}, tdStyle, { textAlign: "right" })}>
                              <TrashBtn onClick={function() { handleDeleteClick(g); }} />
                            </td>
                          </tr>
                        );
                      })}
          ...
          <div style={{ background: "#fff", border: "0.5px solid " + GOLD + "33", borderRadius: 12, overflow: "hidden" }}>
            ...
            {asoebiAll.length === 0 ? (
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr style={{ background: CREAM2 }}>
                      {["Name","Email","Item","Amount","Status","Date",""].map(function(h) {
                        return <th key={h} style={thStyle}>{h}</th>;
                      })}
                    </tr>
                  </thead>
                  <tbody>
                    {asoebiAll.map(function(g, i) {
                      ...
                      return (
                        <tr key={g.id} style={{ background: i % 2 === 0 ? "#fff" : CREAM + "66" }}>
                          ...
                          <td style={Object.assign({}, tdStyle, { color: GOLD, fontFamily: "'Cormorant Garamond',serif", fontSize: 16, fontWeight: 500 })}>
                          ...
                          <td style={Object.assign({}, tdStyle, { color: MUTED, fontSize: 12 })}>
                            {g.created_at ? new Date(g.created_at).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : "--"}
                          </td>
                          <td style={Object.assign({}, tdStyle, { textAlign: "right" })}>
                            <TrashBtn onClick={function() { handleDeleteClick(g); }} />
            ...
            {asoebiAll.length > 0 && (
              <div style={{ padding: "20px 24px", borderTop: "0.5px solid " + GOLD + "22", background: CREAM2, display: "flex", gap: 24, flexWrap: "wrap" }}>
                {[["Total Orders", asoebiAll.length, NAVY],["Paid", paid.length, "#10b981"],["Pending", pending.length, "#f59e0b"]].map(function(item) {
                  return (
                    <div key={item[0]} style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                      <span style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", color: MUTED }}>{item[0]}</span>
                      <span style={{ fontSize: 20, fontFamily: "'Cormorant Garamond',serif", fontWeight: 500, color: item[2] }}>{item[1]}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div>
            ...
            <div style={{ background: "#fff", border: "0.5px solid " + GOLD + "33", borderRadius: 12, padding: "24px" }}>
              ...
              <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                {[
                  [notAttend.length,                   "#ef4444", "Not Attending"],
                  [contributed.length,                 GOLD,      "Chose to Contribute"],
                  [notAttend.length - contributed.length, MUTED,  "Declined to Contribute"],
                ].map(function(item) {
                  return (
                    <div key={item[2]} style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      <span style={{ fontSize: 28, fontFamily: "'Cormorant Garamond',serif", fontWeight: 500, color: item[1] }}>{item[0]}</span>
                      <span style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", color: MUTED }}>{item[2]}</span>
                    </div>
                  );
                })}
              </div>
